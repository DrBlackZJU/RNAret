...
# Argument parser for model and training configuration
...
# Classifier hyperparameters (ResNet-based)
...
parser.add_argument('--ssp_model_path', type=str, default='model/ssp/all_1mer.pth', help='Path to the trained secondary structure prediction model')
...
def constraint(seq):
    """
    Generate a constraint matrix for base pairing.
    Ensures base pairs follow canonical and wobble rules (A-U, C-G, G-U).
    Also applies minimum base pair distance constraint to avoid sharp loops.
    """
    seq = seq.upper()
    L = len(seq)
    ...
    for i in range(sharp, L):
        for j in range(i-sharp):
            ...
            if  ((base_i == 'A' and base_j == 'U') or (base_i == 'U' and base_j == 'A') or
                ...
                base_i == 'N' or base_j == 'N'):  # Allow unknown bases
                matrix[i, j] = 1
...
class outer_concat(torch.nn.Module):
    """
    Concatenate token embeddings in a pairwise outer fashion.
    Produces a tensor representing all pairwise interactions of sequence embeddings.
    """
    def __init__(self):
...
class ResNet2DBlock(torch.nn.Module):
    """
    Basic 2D ResNet block with convolutional layers.
    Uses residual connections to preserve information across layers.
    """
    def __init__(self, embed_dim, kernel_size=3, bias=False):
        super().__init__()
        self.conv_net = torch.nn.Sequential(
            torch.nn.Conv2d(embed_dim, embed_dim, kernel_size=1, bias=bias),
            torch.nn.ReLU(),
            torch.nn.Conv2d(embed_dim, embed_dim, kernel_size=kernel_size, bias=bias, padding="same"),
            torch.nn.ReLU(),
            torch.nn.Conv2d(embed_dim, embed_dim, kernel_size=1, bias=bias),
            torch.nn.ReLU()
        )
...
class ResNet2D(torch.nn.Module):
    """
    2D ResNet composed of multiple ResNet2DBlocks.
    """
...
class ResNet2D_classifier(torch.nn.Module):
    """
    Classifier head built on top of RetNet embeddings.
    Uses outer concatenation + ResNet2D to predict base-pair probabilities.
    """
    def __init__(self):
    ...
    def forward(self, x):
        ...
        x = self.linear_in(x)
        x = x.permute(0, 3, 1, 2)  # Convert to (batch, channel, H, W) format
        x = self.resnet(x)
        x = self.conv_out(x)
...
class rnaret_ssp_model(torch.nn.Module): 
    """
    RNA secondary structure prediction model.
    Uses RetNet encoder + ResNet2D classifier.
    """
    ...
    def forward(self, x):      
        _, aux = self.ret(x)
        x = aux['inner_states'][-1]  # Take last layer hidden states
        x = self.classifier(x)
        return x
...
class post_process(torch.nn.Module):
    ...
    - Apply sigmoid
    - Apply mask
    - Use Hungarian algorithm for optimal base pairing assignment
    - Enforce one-to-one pairing constraints
    ...
    def forward(self, x, mask):
        ...
        # Ensure each base pairs with at most one partner
        for b in range(B):
            for i in range(L):
...
class seq_tokenizer():
    """
    Tokenizer for RNA sequences using k-mers.
    Converts sequences into integer indices.
    """
    ...
    def tokenize(self, seq):
        ...
        seq = seq.upper().replace('U', 'T')  # Convert RNA to DNA alphabet
        seq_len = len(seq)
...
def parse_fasta_file(file):
    """
    Parse a FASTA file and return list of (description, sequence) tuples.
    """
    sequences = []
    description = ""
    seq = ""
...
def generate_dot_bracket(seq_raw, pred):
    """
    Convert predicted base pairs into dot-bracket notation.
    """
    n = len(seq_raw)
    ...
    with torch.no_grad():
        with cm:
            for description, seq_raw in sequences:
                ...
                # Convert prediction to dot-bracket format
