...
parser.add_argument('-i','--train_path', nargs='+', default=['data/data_DeepMirTar_miRAW_noRepeats_3folds_train.txt'], help='Paths to training datasets')
parser.add_argument('-v','--val_path', nargs='+', default=None, help='Paths to validation datasets')
parser.add_argument('-e','--test_path', nargs='+', default=['data/data_DeepMirTar_miRAW_noRepeats_3folds_test.txt'], help='Paths to test datasets')
...
parser.add_argument('--pretrained_model_path', type=str, default=None, help='Path to the pretrained RetNet model')
parser.add_argument('--rri_model_path', type=str, default=None, help='Path to a previously trained RRI model')
parser.add_argument('--use_autocast', action='store_true', help='Enable automatic mixed precision training')
args = parser.parse_args()

# -----------------------------
# Define RRI model architecture
# -----------------------------
class rnaret_rri_model(torch.nn.Module): 
    def __init__(self, args):
        super(rnaret_rri_model, self).__init__()
        self.ret = RetNet(args)
        # Classifier head for binary interaction prediction
    ...
    def forward(self, x): 
        _, aux  = self.ret(x)                           # Forward pass through RetNet
        x = aux['inner_states'][-1]                     # Get last hidden state
        x = x[:, 0, :]                                  # Use CLS token representation
        x = self.classifier(x)                          # Pass through classifier
        return x

# -----------------------------
# Tokenizer for miRNA-mRNA pairs
# -----------------------------
class rri_tokenizer():
    ...
    def read(self, file_path):
        """
        Read dataset file with format:
        [id] [miRNA_seq] [id] [mRNA_seq] [label]
        miRNA sequences are reversed before tokenization.
        """
        miRNA, mRNA, labels = [], [], []
        with open(file_path, 'r') as file:
            ...
            for line in lines:
                line = line.strip()
                parts = line.split()
                miRNA.append(parts[1][::-1])   # Reverse miRNA sequence
                mRNA.append(parts[3])
    ...
    def tokenize(self, seq1, seq2):
        """
        Convert miRNA and mRNA sequences into token IDs
        using k-mer encoding with special tokens.
        """
        ...
        # Generate k-mer tokens for both sequences
        ...
        tokens = np.insert(tokens, 0, 3)  # Insert CLS token at beginning
        return tokens
...
if __name__ == '__main__':
    ...
    if args.train_path is not None and not args.eval_only:
        datasets = [rri_dataset(data_dir, max_len=args.max_len, k=args.k_num) for data_dir in args.train_path]
        train_dataset = torch.utils.data.ConcatDataset(datasets)

        if args.val_path is not None:
            val_datasets = [rri_dataset(data_dir, max_len=args.max_len, k=args.k_num) for data_dir in args.val_path]
    ...
    if args.test_path is not None:
        test_datasets = [rri_dataset(data_dir, max_len=args.max_len, k=args.k_num) for data_dir in args.test_path]
